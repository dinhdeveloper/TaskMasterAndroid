package com.elogictics.taskmaster.common.date_picker
...
import com.elogictics.taskmaster.common.date_picker.model.CalendarEvent
...
class CalendarPicker(context: Context, attrs: AttributeSet) : LinearLayout(context, attrs) {
    ...
    private var calendarPickerListener: CalendarPickerListener? = null

    interface CalendarPickerListener {
        fun onDateSelected(firstDate: Long, secondDate: Long)
    }
    fun setCalendarPickerListener(listener: CalendarPickerListener) {
        calendarPickerListener = listener
    }
    ...
    private val btnChoose: Button by lazy { findViewById(R.id.btnChoose) }
    ...
    init {
        ...
        initCalendar()
        initYearsCalendar()
        initLimitDateSelected()
    }

    private fun initLimitDateSelected() {
    ...
    fun setSecondSelectedDate(year: Int, month: Int, day: Int) {
        ...
        if (firstDate != null && daysBetween(firstDate, calendar.time) <= 30) {
            secondSelectedDate[YEAR_KEY] = year
            secondSelectedDate[MONTH_KEY] = month
            secondSelectedDate[DAY_KEY] = day
        }else{
            Toast.makeText(context, "Vui lòng chọn nhiều nhất 30 ngày", Toast.LENGTH_LONG)
                .show()
        }
    }
    ...
    private fun daysBetween(startDate: Date, endDate: Date): Long {
        ...
        return (endCalendar.timeInMillis - startCalendar.timeInMillis) / millisecondsPerDay
    }
...
//        }
        btnChoose.setOnClickListener {
            // Trong hàm xử lý sự kiện khi người dùng chọn ngày
...
//                val secondDate = DateFormat.getDateInstance().format(Date(selectedDates.second))
                calendarPickerListener?.onDateSelected(selectedDates.first,selectedDates.second)
            }
        }
    }
    ...
    fun initCalendar() {
        yearButton.text =
            "${CalendarUtils.Constants.INT_MONTHS_TO_STRING_MONTHS[currentMonth]}"
        setDateRangeText()
...
//            }
    ...
    private fun setDateRangeText() {
        if (firstSelectedDate[YEAR_KEY] != null) {
            ...
            dateRangeText.text = "$day/$month/$year - dd/mm/yy"
        }
        if (firstSelectedDate[YEAR_KEY] != null && secondSelectedDate.isNotEmpty()) {
            if (secondSelectedDate[YEAR_KEY] == null){
                secondSelectedDate[YEAR_KEY] = firstSelectedDate[YEAR_KEY]
            }
            if (secondSelectedDate[MONTH_KEY] == null){
                secondSelectedDate[MONTH_KEY] = firstSelectedDate[MONTH_KEY]
            }
            if (secondSelectedDate[DAY_KEY] == null){
                secondSelectedDate[DAY_KEY] = firstSelectedDate[DAY_KEY]
            }
            setDateRangeChoose()
        }
        if (firstSelectedDate[YEAR_KEY] != null && secondSelectedDate[YEAR_KEY] != null){
    ...
    private fun setDateRangeChoose(){
        val firstMonth = "${
            if (firstSelectedDate[MONTH_KEY]!!.plus(1) < 10) "0${
                firstSelectedDate[MONTH_KEY]!!.plus(
                    1
        ...
        dateRangeText.text =
            "$firstDay/$firstMonth/$firstYear - $secondDay/$secondMonth/$secondYear"
    }
    ...
    private fun getFirstCurrentDate() : Triple<String,String,String>{
        val firstMonth = "${
            if (firstSelectedDate[MONTH_KEY]!!.plus(1) < 10) "0${
                firstSelectedDate[MONTH_KEY]!!.plus(
                    1
        ...
        val firstDay =
            "${if (firstSelectedDate[DAY_KEY]!! < 10) "0${firstSelectedDate[DAY_KEY]}" else firstSelectedDate[DAY_KEY]}"
        val firstYear = "${firstSelectedDate[YEAR_KEY]}"
        return Triple(firstYear, firstMonth, firstDay)
    }
    private fun getSecondCurrentDate() : Triple<String,String,String>{
        ...
        return Triple(secondYear, secondMonth, secondDay)
    }
                    ...
                    val layers = if (tvsWithEvent.contains(tv.text.toString())) {
                        eventDotGravity?.let {
                            when (it) {
                                ...
                                Gravity.END -> {
                                    val (firstYear, firstMonth, firstDay) = getFirstCurrentDate()
                                    val (secondYear, secondMonth, secondDay) = getSecondCurrentDate()
                                    if (firstDay == secondDay && firstMonth == secondMonth && firstYear == secondYear){
                                        ContextCompat.getDrawable(
                                            context,
                                            R.drawable.circle_center_event
                                        ) as LayerDrawable?
                                    }else{
                                        ContextCompat.getDrawable(
                                            context,
            ...
            if (fromDate != "dd/mm/yyyy") {
                if (fromDate.length == 10) {
                    try {
                        ...
                        // false
                        Toast.makeText(context, "Không thể phân tích ngày bắt đầu", Toast.LENGTH_LONG)
                            .show()
                    ...
                    // false
                    Toast.makeText(context, "Ngày bắt đầu không hợp lệ", Toast.LENGTH_LONG).show()
                    return Pair(false, Pair(0L, 0L))
                }
            } else {
                // false
                Toast.makeText(context, "Nhập ngày bắt đầu", Toast.LENGTH_LONG).show()
                return Pair(false, Pair(0L, 0L))
            }
            ...
            if (toDate != "dd/mm/yyyy") {
                if (toDate.length == 10) {
                    return try {
                        ...
                        Log.e(TAG, e.localizedMessage ?: "error" + "/n" + e.stackTrace.toString())
                        // false
                        Toast.makeText(context, "Không thể phân tích ngày kết thúc", Toast.LENGTH_LONG)
                            .show()
                    ...
                    // false
                    Toast.makeText(context, "Ngày kết thúc không hợp lệ", Toast.LENGTH_LONG).show()
                    return Pair(false, Pair(0L, 0L))
                }
            } else {
                // false
                Toast.makeText(context, "Nhập ngày kết thúc", Toast.LENGTH_LONG).show()
                return Pair(false, Pair(0L, 0L))
            }
        }
    }
