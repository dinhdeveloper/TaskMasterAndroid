package com.elogictics.taskmaster.common.widgets.tagview.utils;
...
import com.elogictics.taskmaster.common.widgets.tagview.models.TagIndex;
    ...
    public static void parseText(Editable s, List<TagInterface> tags) {
        ...
        for (int i = 0; i < s.length(); i++) {
            if (list.indexOfKey(i) > -1) {
                ...
                s.setSpan(
                        new TextReplacementSpan(
                        tagIndex.getTag(),
                        tagIndex.getLabel()),
                        tagIndex.getStart(),
                        tagIndex.getStart() + tagIndex.getCount(),
